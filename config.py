...
POSTGRES_DB = os.getenv(
    "POSTGRES_DB", "instagram_embeddings"
)
...
BACKFILL_START_DATE = (
    os.getenv("BACKFILL_START_DATE") or None
)
BACKFILL_END_DATE = (
    os.getenv("BACKFILL_END_DATE") or None
)
...
def validate_config() -> None:
    ...
    if BACKFILL_MODE and not BACKFILL_START_DATE:
        raise ValueError(
            "BACKFILL_START_DATE is required "
            "when BACKFILL_MODE=true"
        )
    if BACKFILL_BATCH_DAYS <= 0:
        raise ValueError(
            "BACKFILL_BATCH_DAYS must be greater than 0"
        )
